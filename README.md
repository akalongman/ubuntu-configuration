...
- [Optional Configurations](#optional-configurations)
    ...
    - [Mounting Drives](#mounting-drives)
        ...
        - [Mounting of Network Drives](#mounting-of-network-drives)    - [SSH](#ssh)
    ...
    - [Development](#development)
        - [LAMP](#lamp)
            ...
            - [Apache](#apache)
                ...
                - [Enable php-mcrypt](#apache-enable-php-mcrypt--mod_rewrite)
...
and, if successful  
    ...
    ------------------------------------
    sec   rsa4096/3AA5C34371567BD2 2026-03-10 [expires: 2028-03-10]
    uid                          Hubot 
    ssb   rsa4096/42B317FD4BA89E7A 2026-03-10
...
GnuPG 2.1+ (Ubuntu ships 2.4) no longer stores keys in `secring.gpg`. Export them instead:

    gpg --export --armor $GPGKEY > public-gpg.asc
    gpg --export-secret-keys --armor $GPGKEY > secret-gpg.asc
    gpg --export-ownertrust > ownertrust-gpg.txt
    
### Restore The GPG Key

    gpg --import public-gpg.asc
...
Dev version:
    ...
    echo "deb [signed-by=/usr/share/keyrings/sublimehq-archive.gpg] https://download.sublimetext.com/ apt/dev/" | sudo tee /etc/apt/sources.list.d/sublime-text.list    
...
Installation
    ...
    curl -fsSL https://download.spotify.com/debian/pubkey_C85668DF69375001.gpg | sudo gpg --dearmor -o /usr/share/keyrings/spotify.gpg
...
Python is installed by default on Ubuntu, version 3.12 is suitable. I strongly recommend installing `python3-dev` for headers to compile certain PIP packages.

    sudo apt install -y python3-dev python3-full python3-venv pipx libmysqlclient-dev

Ubuntu 24.04 marks the system Python as externally managed (PEP 668), so install command line tools with `pipx` and project libraries inside a virtualenv:
...
#### Virtualenv
Virtualenv creates isolated Python environments. Ubuntu 24.04 ships the built-in `venv` module, so no global install is needed.
...
For installation Yarn package manager run:
    ...
    curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | sudo gpg --dearmor -o /usr/share/keyrings/yarn-archive.gpg
    echo "deb [signed-by=/usr/share/keyrings/yarn-archive.gpg] https://dl.yarnpkg.com/debian/ stable main" | sudo tee /etc/apt/sources.list.d/yarn.list
